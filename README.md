# !!! ВНИМАНИЕ: это форк ветки stable2 !!!

- Конкретно в этом форке - добавлены домены дискорда для general.bat. Это позволит одновременно пользоватся дискордом в звонке и чате, а также ютубом. Причём без потребности запускать отдельный discord.bat файл. Также в list-general.txt добавлены новые домены, для обхода остальных блокировок. 

- Также настоятельно рекомендую установить пакет warp-cli для создания VPN-туннеля с сетью Cloudflare, что также может помочь в обходе блокировок.




# - Далее идёт информация из оригинального репозитория -
