...
- Конкретно в этом форке добавлены домены дискорда для general.bat. Это позволит одновременно пользоватся дискордом в звонке и чате, а также ютубом. Причём без потребности запускать отдельный discord.bat файл.
